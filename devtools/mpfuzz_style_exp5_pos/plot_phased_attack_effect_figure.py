...
def render(
    ...
    source_note = "Included counts are corrected from chain tx hashes when RPC is available; txpool gas is a weighted estimate from observed txpool counts."
    attack_zero_payload = (
    ...
    out: list[str] = [
        ...
        text(left, 76, "Baseline sends only normal transactions; the attack run injects a two-block Deter-Z2 burst while normal traffic continues.", 13, color="muted"),
    ...
    max_attack_tick = max(ticks, key=lambda tick: attack_tx.get(tick, 0.0))
    max_attack_value = attack_tx.get(max_attack_tick, 0.0)
    if max_attack_value > 0:
        out.append(
            annotate(
                xscale.x(max_attack_tick) + 12,
                max(top.y + 42, top.y_value(max_attack_value) - 12),
                f"{int(max_attack_value)} first-0 attack tx included",
                "attack_tx",
            )
        )
    stall_ticks = [
