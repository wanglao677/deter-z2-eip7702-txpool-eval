...
def receipt_blocks_fee_by_tick(run_dir: Path, rows: list[dict[str, str]], kind: str) -> dict[int, int]:
    ...
    for row in read_csv(path):
        ...
        if tick in ticks:
            fees_by_tick[tick] += as_int(row.get(field))
    return dict(fees_by_tick)
...
def fee_by_tick(
    ...
    kind = "attack" if record_file.startswith("attack") else "normal"
    from_receipt_blocks = receipt_blocks_fee_by_tick(run_dir, rows, kind)
    if from_receipt_blocks:
        return from_receipt_blocks, "fee-receipt-blocks"
